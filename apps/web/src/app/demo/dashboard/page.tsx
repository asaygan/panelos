...
import { demoActivity, demoLocations, demoPanels, demoRevQueue } from "@/lib/demo/data";
  ...
  return (
    <Card style={{ padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
        <span className="mono" style={{ fontSize: 26, fontWeight: 700, letterSpacing: "-.02em", lineHeight: 1 }}>
          {value}
        </span>
        ...
        {delta && (
          <span
            ...
            style={{
              ...
              padding: "0 5px",
              background:
                tone === "fault" ? "var(--c-fault-soft)" : tone === "ok" ? "var(--c-ok-soft)" : "var(--c-idle-soft)",
              color: tone === "fault" ? "var(--c-fault)" : tone === "ok" ? "var(--c-ok)" : "var(--c-ink-3)",
            }}
          >
            {delta}
          </span>
...
function Spark({ data, tone }: { data: number[]; tone: "ok" | "fault" | "idle" }) {
  ...
  const pts = data
    .map((d, i) => `${(i / (data.length - 1)) * w},${h - ((d - min) / (max - min || 1)) * (h - 3) - 1.5}`)
    .join(" ");
...
export default function DemoDashboardPage() {
  ...
  // "Attention" now = anything not yet operational (still in design or archived).
  ...
  return (
    <Page>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.7fr 1fr", gap: "var(--gap)" }}>
        <Card>
          <CardHead
            ...
            actions={
              <Btn size="sm" variant="ghost" icon="external-link" onClick={() => router.push("/demo/panels")}>
                All panels
              </Btn>
            }
          />
          <table className="tbl">
            ...
            <tbody>
              {attention.map((p) => (
                <tr key={p.id} style={{ cursor: "pointer" }} onClick={() => router.push(`/demo/panels/${p.id}`)}>
                  <td>
                    <span className="strong">{p.tag}</span> <span style={{ color: "var(--c-ink-3)" }}>· {p.name}</span>
                  </td>
                  <td>{p.loc}</td>
          ...
          <div style={{ padding: 10, borderTop: "1px solid var(--c-line)", display: "flex", gap: 8 }}>
            <Btn size="sm" icon="plus" variant="primary" onClick={() => router.push("/demo/panels")}>
              Add panel
            </Btn>
            <Btn size="sm" icon="qr-code" onClick={() => router.push("/demo/labels")}>
              Generate labels
            </Btn>
            <Btn size="sm" icon="git-branch" onClick={() => router.push("/demo/revisions")}>
              Review revisions
            </Btn>
          </div>
        </Card>
        ...
        <Card style={{ display: "flex", flexDirection: "column" }}>
          <CardHead
            title="Activity"
            actions={
              <Btn size="sm" variant="ghost" icon="activity" onClick={() => router.push("/demo/revisions")}>
                Log
              </Btn>
            }
          />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: "var(--gap)", marginTop: "var(--gap)" }}>
        <Card>
          ...
          <div style={{ padding: 12, display: "flex", flexDirection: "column", gap: 11 }}>
            {locations.map((l) => {
              ...
              return (
                <div key={l.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "var(--fz-sm)", marginBottom: 4, gap: 8 }}>
                    <span style={{ color: "var(--c-ink-2)", fontWeight: 540, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {l.name}
                    </span>
                  ...
                  <div style={{ height: 6, background: "var(--c-surface-3)", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{ width: `${pct}%`, height: "100%", background: "var(--c-accent)", borderRadius: 3 }} />
                  </div>
                </div>
              );
            })}
        ...
        <Card>
          <CardHead
            ...
            actions={
              <Btn size="sm" variant="ghost" onClick={() => router.push("/demo/revisions")}>
                Open queue
              </Btn>
            }
          />
          <table className="tbl">
            ...
            <tbody>
              {queue.map((r) => (
                <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => router.push("/demo/revisions")}>
